using System;
using System.Collections.Generic;
using System.Linq;
using System.Text;

namespace EnoGateway
{
    public static class CRC8
    {
        //
        // CRC8 calculation
        // ================
        //
        // The polynomial G(x) = x^8 + x^2 + x^1 + x^0 is used to generate the CRC8 table, needed for the CRC8 calculation
        //
        private static byte[] CRC8Table = { 
                                          };
        /// <summary>
        /// Return the CRC8 using the CRC8 table
        /// </summary>
        /// <param name="CRC8"></param>
        /// <param name="data"></param>
        /// <returns></returns>
        private static byte GetCRC8FromTable(byte CRC8, byte data)
        {
            return CRC8Table[(CRC8 ^ data) & 0xff];
        }
        /// <summary>
        /// Return the CRC8 for the header of the telegram (frame[1]...frame[4])
        /// </summary>
        /// <param name="frame"></param>
        /// <returns></returns>
        public static byte GetCRC8Header(byte[] frame)
        {
            byte CRC8 = 0;
            for (int i = 1; i < 5; i++)
            {
                CRC8 = GetCRC8FromTable(CRC8, frame[i]);
            }
            return CRC8;
        }
        /// <summary>
        /// Return the CRC8 for the data of the telegram (frame[6]...frame[frameLength-1])
        /// </summary>
        /// <param name="frame"></param>
        /// <returns></returns>
        public static byte GetCRC8Data(byte[] frame)
        {
            byte CRC8 = 0;
            for (int i = 6; i < (frame.Length - 1); i++)
            {
                CRC8 = GetCRC8FromTable(CRC8, frame[i]);
            }
            return CRC8;
        }
        public static byte GetCRC8Data(byte[] frame, int length)
        {
            byte CRC8 = 0;
            for (int i = 6; i < length; i++)
            {
                CRC8 = GetCRC8FromTable(CRC8, frame[i]);
            }
            return CRC8;
        }
        /// <summary>
        /// Recalculate the C8Header and the CRC8Data in the frame (the frame must be passed by reference)
        /// </summary>
        /// <param name="frame"></param>
        public static void SetAllCRC8(ref byte[] frame)
        {
            frame[5] = GetCRC8Header(frame);
            frame[frame.Length - 1] = GetCRC8Data(frame);
        }
    }
}
